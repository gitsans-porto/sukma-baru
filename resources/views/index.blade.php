...
<body id="page-top">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3 d-flex justify-content-between align-items-center">
                            ...
                            <div class="d-flex gap-2">
                                ...
                                <button type="button" id="editModeBtn" class="btn btn-outline-warning btn-sm" onclick="toggleEditMode()">
                                    <i class="fas fa-edit"></i> Pilih
                                </button>
                                <!-- Delete Selected Button (Initially Hidden) -->
                                <button type="button" id="deleteSelectedBtn" class="btn btn-danger btn-sm d-none" onclick="deleteSelected()">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover" id="pendudukTable">
                                        <thead class="table-primary">
                                            <tr>
                                                <th style="width: 40px;" class="text-center align-middle edit-mode-col d-none">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach($groupedPenduduk as $noKk => $anggotaKeluarga)
                                                ...
                                                @foreach($anggotaKeluarga as $penduduk)
                                                    <tr>
                                                        @if($isFirstRow)
                                                            <td rowspan="{{ $jumlahAnggota }}" class="text-center align-middle valign-middle edit-mode-col d-none" style="vertical-align: middle;">
                                                                <div class="form-check">
                                                                    <input type="checkbox" class="form-check-input family-checkbox"
                                                                           id="family_{{ $kartuKeluargaId }}"
                                                                           data-family-id="{{ $kartuKeluargaId }}"
                                                                           onchange="toggleFamilySelection({{ $kartuKeluargaId }})"
                                                                           title="Pilih Semua Anggota Keluarga">
                                                                    <label class="form-check-label" for="family_{{ $kartuKeluargaId }}"></label>
                                                                </div>
                                                            </td>
                                                        ...
                                                        <td class="align-middle">
                                                            <div class="d-flex align-items-center">
                                                                <!-- Individual Checkbox (Hidden by default) -->
                                                                <div class="form-check me-2 edit-mode-col d-none">
                                                                    <input type="checkbox" class="form-check-input individual-checkbox"
                                                                           id="penduduk_{{ $penduduk->id }}"
                                                                           value="{{ $penduduk->id }}"
                                                                           data-family-id="{{ $kartuKeluargaId }}"
                                                                           onchange="updateDeleteButton()">
                                                                    <label class="form-check-label" for="penduduk_{{ $penduduk->id }}"></label>
                                                                </div>
                                                                <div>
                                                                    {{ $penduduk->nama }}
    ...
    <script>
        ...
        function hapusKeluarga(kartuKeluargaId) {
            if (confirm('Apakah Anda yakin ingin menghapus seluruh data keluarga ini?')) {
        ...
        function toggleEditMode() {
            ...
            const editModeCols = document.querySelectorAll('.edit-mode-col');

            if (editMode) {
                // Enter edit mode
                editModeBtn.innerHTML = '<i class="fas fa-times"></i> Batal';
                editModeBtn.classList.remove('btn-outline-warning');
                editModeBtn.classList.add('btn-outline-secondary');
                deleteSelectedBtn.classList.remove('d-none');
                editModeCols.forEach(col => col.classList.remove('d-none'));
            } else {
                // Exit edit mode
                editModeBtn.innerHTML = '<i class="fas fa-edit"></i> Pilih';
                editModeBtn.classList.remove('btn-outline-secondary');
                editModeBtn.classList.add('btn-outline-warning');
                deleteSelectedBtn.classList.add('d-none');
                editModeCols.forEach(col => col.classList.add('d-none'));
        ...
        function toggleSelectAll() {
            ...
            const familyCheckboxes = document.querySelectorAll('.family-checkbox');
        ...
        function toggleFamilySelection(familyId) {
            const familyCheckbox = document.getElementById('family_' + familyId);
            const individualCheckboxes = document.querySelectorAll(`.individual-checkbox[data-family-id="${familyId}"]`);

            individualCheckboxes.forEach(checkbox => {
                checkbox.checked = familyCheckbox.checked;
            });
        ...
        function updateSelectAllCheckbox() {
            ...
            const totalFamilies = familyCheckboxes.length;
            const checkedFamilies = document.querySelectorAll('.family-checkbox:checked').length;
            const totalIndividuals = individualCheckboxes.length;
            const checkedIndividuals = document.querySelectorAll('.individual-checkbox:checked').length;

            selectAll.checked = (totalFamilies === checkedFamilies && totalIndividuals === checkedIndividuals && totalIndividuals > 0);
        }
        ...
        function updateDeleteButton() {
            ...
            if (checkedCount > 0) {
                deleteSelectedBtn.innerHTML = `<i class="fas fa-trash"></i> Hapus (${checkedCount})`;
                deleteSelectedBtn.disabled = false;
            } else {
                deleteSelectedBtn.innerHTML = '<i class="fas fa-trash"></i> Hapus yang Dipilih';
        ...
        function clearAllSelections() {
            ...
            familyCheckboxes.forEach(checkbox => checkbox.checked = false);
        ...
        function deleteSelected() {
            const selectedIndividuals = document.querySelectorAll('.individual-checkbox:checked');
            const selectedIds = Array.from(selectedIndividuals).map(cb => cb.value);
            ...
            if (confirm(`Apakah Anda yakin ingin menghapus ${selectedIds.length} data penduduk yang dipilih? Data akan dihapus permanen dan tidak dapat dikembalikan.`)) {
                // Create form for bulk delete
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = '/penduduk/bulk-delete';
                form.style.display = 'none';
        ...
        // Handle individual checkbox changes to update family checkbox state
        document.addEventListener('change', function(e) {
            if (e.target.classList.contains('individual-checkbox')) {
                const familyId = e.target.dataset.familyId;
                const familyCheckbox = document.getElementById('family_' + familyId);
                const familyIndividualCheckboxes = document.querySelectorAll(`.individual-checkbox[data-family-id="${familyId}"]`);

                const totalInFamily = familyIndividualCheckboxes.length;
                const checkedInFamily = document.querySelectorAll(`.individual-checkbox[data-family-id="${familyId}"]:checked`).length;

                familyCheckbox.checked = (totalInFamily === checkedInFamily);
